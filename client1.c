...
int thread_send_message(void *argv)
{
    ...
    while (1)
    {
        ...
        {
            printf("send the message to the server is successful!\n");
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    local_client1.len = sizeof(struct sockaddr_in); //init the server address
    if (connect(local_client1.local_socket, (struct sockaddr *)&local_client1.server_addr, local_client1.len) >= 0)
    {
        printf("connect the server is successful!\n");
